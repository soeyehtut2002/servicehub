...
import { LayoutDashboard, User, MessageSquare, Plus, LogOut, ChevronDown, ChevronUp, Sun, Moon, Globe } from 'lucide-react';
  ...
  return (
    <nav className={`navbar ${scrolled ? 'navbar-scrolled' : ''}`}>
      ...
      {menuOpen && (
        <div className="mobile-menu">
          <div className="mobile-search">
            <SearchAutocomplete placeholder={t('nav_search_ph')} onClose={() => setMenuOpen(false)} />
          </div>
          <Link to="/services" className="mobile-link" onClick={() => setMenuOpen(false)}>{t('nav_services')}</Link>
          {user ? (
            <>
              <Link to={getDashboardLink()} className="mobile-link" onClick={() => setMenuOpen(false)}>{t('nav_dashboard')}</Link>
              <button className="mobile-link danger-link" onClick={handleLogout}>{t('nav_signout')}</button>
            </>
          ) : (
            <>
              <Link to="/login" className="mobile-link" onClick={() => setMenuOpen(false)}>{t('nav_signin')}</Link>
              <Link to="/register" className="mobile-link" onClick={() => setMenuOpen(false)}>{t('nav_getstarted')}</Link>
            </>
          )}
        </div>
      )}
      ...
      <style>{`
        ...
        .search-input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px var(--primary-glow); background: var(--bg-card); }
        ...
        .chat-badge {
          ...
          border-radius: 10px; padding: 1px 5px; border: 2px solid var(--bg-card); min-width: 18px; text-align: center;
        }
        ...
        .danger-link:hover { color: var(--danger); background: rgba(239,68,68,0.08); }
